    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.memberId) {
            this.$http({
              url: this.$http.adornUrl(`/member/storevideorela/info/${this.dataForm.memberId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.videoId = data.storeVideoRela.videoId
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/member/storevideorela/${!this.dataForm.memberId ? 'save' : 'update'}`),
              method: 'post',
