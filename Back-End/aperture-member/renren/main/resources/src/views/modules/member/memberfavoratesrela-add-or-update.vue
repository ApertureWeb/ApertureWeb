    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.memberId) {
            this.$http({
              url: this.$http.adornUrl(`/member/memberfavoratesrela/info/${this.dataForm.memberId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.favoratesId = data.memberFavoratesRela.favoratesId
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/member/memberfavoratesrela/${!this.dataForm.memberId ? 'save' : 'update'}`),
              method: 'post',
